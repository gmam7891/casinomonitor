...
with abas[5]:
    ...
    if df_geral.empty:
        ...
        st.success("✅ Dados carregados para análise.")

        # Corrigir hora para Brasil
        ...
        # Filtro de datas selecionadas (pode usar filtros externos ou definir padrão)
        ...
        # Filtro por jogo
        jogos_unicos = df_filtrado["jogo_detectado"].dropna().unique()
        jogo_selecionado = st.selectbox("🎮 Filtrar por Jogo:", options=["Todos"] + list(jogos_unicos))
        if jogo_selecionado != "Todos":
            df_filtrado = df_filtrado[df_filtrado["jogo_detectado"] == jogo_selecionado]

        # Exportar CSV
        st.download_button(
            label="⬇️ Baixar CSV Detecções Filtradas",
            data=df_filtrado.to_csv(index=False).encode("utf-8"),
            file_name="detecções_filtradas.csv",
            mime="text/csv"
        )
        ...
        if "jogo_detectado" in df_filtrado.columns:
            ...
            ranking.columns = ["Jogo", "Aparições"]
            fig1 = px.pie(ranking, names="Jogo", values="Aparições", title="Distribuição dos Jogos Detectados")
            st.plotly_chart(fig1, use_container_width=True)

        # --- Gráfico 2: Detecções por Streamer ---
        st.markdown("### 🧍‍♂️ Comparativo: Total de Detecções por Streamer")
        if "streamer" in df_filtrado.columns:
            comparativo = df_filtrado.groupby("streamer").size().reset_index(name="Detecções")
            fig2 = px.bar(comparativo, x="streamer", y="Detecções", title="Total de Detecções por Streamer", text_auto=True)
        ...
        fig3 = px.line(evolucao, x="data_brasilia", y="Detecções", color="jogo_detectado", title="Detecções ao Longo do Tempo")
        st.plotly_chart(fig3, use_container_width=True)

        # --- Gráfico 4: Tempo Médio de Detecção por Jogo ---
        st.markdown("### ⏱ Tempo Médio de Detecção por Jogo")
        if "segundo" in df_filtrado.columns:
            media_tempo = df_filtrado.groupby("jogo_detectado")["segundo"].mean().reset_index()
            fig4 = px.bar(media_tempo, x="jogo_detectado", y="segundo", title="Tempo Médio de Detecção por Jogo", text_auto=".2f")
        ...
        fig5 = px.bar(top_streamers, x="jogo_detectado", y="Detecções", color="streamer", barmode="group", title="Top Streamers por Jogo")
        st.plotly_chart(fig5, use_container_width=True)

        # --- Gráfico 6: Detecções por Dia da Semana ---
        st.markdown("### 📆 Detecções por Dia da Semana")
        df_filtrado["dia_semana"] = df_filtrado["data_brasilia"].dt.dayofweek.map({
            0: "segunda-feira", 1: "terça-feira", 2: "quarta-feira",
            3: "quinta-feira", 4: "sexta-feira", 5: "sábado", 6: "domingo"
        })
        distrib_dia = df_filtrado["dia_semana"].value_counts().reindex([
            "segunda-feira", "terça-feira", "quarta-feira", "quinta-feira", "sexta-feira", "sábado", "domingo"
        ]).fillna(0).reset_index()
        distrib_dia.columns = ["Dia", "Detecções"]
        fig6 = px.bar(distrib_dia, x="Dia", y="Detecções", title="Detecções por Dia da Semana", text_auto=True)
        st.plotly_chart(fig6, use_container_width=True)

        # --- Gráfico 7: Mapa de Calor Jogo x Dia ---
        st.markdown("### 🔥 Mapa de Calor: Jogos por Dia da Semana")
        matriz = (
            df_filtrado.groupby(["jogo_detectado", "dia_semana"])
            .size().unstack(fill_value=0).reindex(columns=[
                "segunda-feira", "terça-feira", "quarta-feira", "quinta-feira", "sexta-feira", "sábado", "domingo"
            ], fill_value=0)
        )
        fig7 = px.imshow(matriz, labels=dict(x="Dia", y="Jogo", color="Detecções"), aspect="auto", color_continuous_scale="Oranges")
        st.plotly_chart(fig7, use_container_width=True)

        # --- Gráfico 8: Tendência de Crescimento (Média Móvel) ---
        st.markdown("### 📈 Tendência de Crescimento por Jogo")
        tendencia = (
            df_filtrado.groupby([pd.Grouper(key="data_brasilia", freq="D"), "jogo_detectado"])
            .size().reset_index(name="Detecções")
        )
        tendencia["Média Móvel"] = tendencia.groupby("jogo_detectado")["Detecções"].transform(lambda x: x.rolling(window=3, min_periods=1).mean())
        fig8 = px.line(tendencia, x="data_brasilia", y="Média Móvel", color="jogo_detectado", title="Tendência Média de Detecção")
        st.plotly_chart(fig8, use_container_width=True)

        # --- Gráfico 9: Média de Viewers por Jogo ---
        if "viewers" in df_filtrado.columns:
            st.markdown("### 👀 Média de Viewers por Jogo")
            media_viewers = df_filtrado.groupby("jogo_detectado")["viewers"].mean().reset_index()
            fig9 = px.bar(media_viewers, x="jogo_detectado", y="viewers", title="Audiência Média por Jogo", text_auto=".0f")
            st.plotly_chart(fig9, use_container_width=True)

        # --- Painel de Últimas Lives ---
        st.markdown("### 🔴 Últimas Detecções")
        ultimas = df_filtrado.sort_values(by="data_brasilia", ascending=False).head(5)
        for _, row in ultimas.iterrows():
            st.markdown(f"- **{row['streamer']}** jogando **{row['jogo_detectado']}** – {row['data_brasilia'].strftime('%d/%m %H:%M')}")

        # --- Análise de Horário das Lives ---
        st.markdown("### ⏰ Distribuição dos Horários de Lives")
        df_filtrado["hora"] = df_filtrado["data_brasilia"].dt.hour
        ...
        if "categoria" in df_filtrado.columns:
            ...
            fig_categorias = px.pie(
                ...
                color_discrete_sequence=["#FF7F0E", "#1F77B4", "#2CA02C", "#D62728", "#9467BD"]
            )

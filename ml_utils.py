...
def varrer_url_customizada(url, st, session_state, prever_func, skip_inicial=0, intervalo=1000, max_frames=10000):
    ...
    for i in range(max_frames):
        ...
        if not capturar_frame_ffmpeg_imageio(url, frame_path, skip_seconds=tempo_atual):
            st.error(f"❌ Falha ao capturar frame no segundo {tempo_atual}. Interrompendo varredura.")
            break
    ...
    if resultados:
        st.success(f"✅ Varredura finalizada. Total de jogos detectados: {len(resultados)}")
    else:
        st.warning("⚠️ Nenhum jogo detectado na varredura.")
